...
class Ronda1Screen extends StatefulWidget {
  final String sesionId;
  final List<VocationalQuestion> preguntas;
  const Ronda1Screen({super.key, required this.sesionId, required this.preguntas});

  @override
  State<Ronda1Screen> createState() => _Ronda1ScreenState();
}

class _Ronda1ScreenState extends State<Ronda1Screen> {
  ...
  Future<void> _enviar() async {
    ...
    try {
      final res = await _service.guardarRespuestasRonda1(
          widget.sesionId, _respuestas.values.toList());
      if (!mounted) return;
      Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (_) =>
            Ronda2Screen(sesionId: widget.sesionId, preguntas: res.preguntasRonda2),
      ));
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ronda 1 – Holland RIASEC'),
        leading: IconButton(
      ...
      body: Column(children: [
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              RondaLabel(label: 'RONDA 1', color: AppColors.primary),
        ...
        TestNavBar(
          ...
          esUltima: esUltima,
          enviando: _enviando,
          labelFinalizar: 'Finalizar Ronda 1',
        ),
      ]),
    );
  }
